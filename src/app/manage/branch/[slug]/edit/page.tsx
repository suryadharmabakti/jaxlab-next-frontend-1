...
export default function Page() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const result = await fetch(`/api/branch/${id}`, {
        method: "PUT",
        headers: {
        ...
        body: JSON.stringify({
          idOwner: user._id,
          name: draft.name,
          noTeleponAdmin: draft.noTeleponAdmin,
          alamat: draft.alamat,
        }),
      });
      ...
      if (!result.ok) throw new Error("Failed to update branch data.");
  ...
  const fetchById = async () => {
    try {
      ...
      if (!res.ok) throw new Error("Failed to retrieve branch data");
      ...
      setDraft({
        _id: data.data._id,
        name: data.data.name,
        noTeleponAdmin: data.data.noTeleponAdmin,
