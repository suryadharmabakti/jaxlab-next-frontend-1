...
export default function Page() {
  ...
  const [draft, setDraft] = useState<Category>({ _id: "", name: "", code: "" });
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const result = await fetch(`/api/product-category/${id}`, {
        method: "PUT",
        headers: {
      ...
      if (!result.ok)
        throw new Error("Failed to update product category data.");

      toast.success("Product category successfully updated.");
  ...
  const fetchById = async () => {
    try {
      ...
      if (!res.ok) throw new Error("Failed to retrieve product category data");
        ...
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-3 mb-5">
            ...
            <div className="text-xs text-gray-400">
              Kelola Cabang &nbsp;›&nbsp; Kategori Produk &nbsp;›&nbsp; Edit
            </div>
          </div>
        </div>
      </div>
